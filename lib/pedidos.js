...
export function inicioDeHoyPY() {
  // Medianoche en Paraguay → convertir a UTC para la query
  const ahoraUtc = Date.now();
  const ahoraPy  = new Date(ahoraUtc + PY_OFFSET_MS);
  // Truncar a medianoche en tiempo de Paraguay
  const medianochePy = new Date(
    ahoraPy.getUTCFullYear(),
    ahoraPy.getUTCMonth(),
    ahoraPy.getUTCDate(),
    0, 0, 0, 0
  );
  // Devolver como UTC restando el offset
  return new Date(medianochePy.getTime() - PY_OFFSET_MS);
}

// ─── Listado de pedidos de un vendedor ───────────────────────────────────────
// periodo: 'hoy' (desde medianoche PY) | 'semana' (últimos 7 días)
