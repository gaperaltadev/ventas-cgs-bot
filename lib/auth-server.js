...
<div class="card">
  ...
  <div id="vincular-view" style="display:none;">
    ...
    <div id="circuit-banner" style="display:none;background:#FEF2F2;border:2px solid #DC2626;border-radius:8px;padding:16px;margin-bottom:16px;">
      ...
      <div style="background:#fff;border-radius:6px;padding:10px 12px;font-size:12px;color:#5B6478;margin-bottom:12px;">
      ...
      <button id="reset-btn" style="background:#DC2626;color:white;border:none;padding:10px 20px;border-radius:6px;font-weight:700;font-size:13px;cursor:pointer;width:100%;">
        Reset manual (limpiar y reintentar)
      </button>
      <div id="reset-status" style="font-size:12px;color:#047857;margin-top:8px;display:none;"></div>
    </div>
...
<script>
  ...
  // Handler del botón Reset
  document.addEventListener('DOMContentLoaded', () => {
    const resetBtn = document.getElementById('reset-btn');
    if (resetBtn) {
      resetBtn.addEventListener('click', async () => {
        if (!confirm('¿Confirmás el reset? Esperá al menos 24-48h desde el último intento antes de hacerlo.')) return;
        ...
        try {
          ...
          resetBtn.textContent = 'Reset manual (limpiar y reintentar)';
        }
      });
    }
  });
