...
// Set externamente desde index.js via setResetHandler.
// Es un callback porque no podemos importar directamente (circular dep).
let _resetHandler = null;
let _getCircuitState = () => ({ open: false, openedAt: 0, pauseMs: 0 });

export function setResetHandler(fn) { _resetHandler = fn; }
export function setCircuitGetter(fn) { _getCircuitState = fn; }
    ...
    try {
      ...
      if (url.pathname === '/api/state') {
        ...
        const phoneMasked = state.phoneNumber
          ...
          : null;
        const circuit = _getCircuitState();
        res.end(JSON.stringify({
          ...
          circuit: {
            open: circuit.open,
            opensIn: circuit.open
              ? Math.max(0, (circuit.openedAt + circuit.pauseMs) - Date.now())
              : 0
          }
        }));
        return;
      ...
      // Reset manual: limpia auth_info y reactiva el bot
      if (url.pathname === '/api/reset' && req.method === 'POST') {
        if (!_resetHandler) {
          res.writeHead(503, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: 'reset handler no configurado' }));
          return;
        }
        try {
          await _resetHandler();
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true, message: 'Reset ejecutado. El bot va a reintentar en unos segundos.' }));
        } catch (e) {
...
<div class="card">
  ...
  <div id="vincular-view" style="display:none;">
    ...
    <div id="circuit-banner" style="display:none;background:#FEF2F2;border:2px solid #DC2626;border-radius:8px;padding:16px;margin-bottom:16px;">
      <div style="font-weight:700;color:#DC2626;font-size:14px;margin-bottom:6px;">🚫 Bot pausado para prevenir ban permanente</div>
      <div id="circuit-detail" style="font-size:13px;color:#7F1D1D;line-height:1.5;margin-bottom:12px;"></div>
      <div style="background:#fff;border-radius:6px;padding:10px 12px;font-size:12px;color:#5B6478;">
        <strong>Antes de hacer reset:</strong> esperá al menos 24-48h desde el último intento.
        Si reseteás demasiado rápido y el número sigue baneado, vas a empeorar la situación.
      </div>
    </div>
    ...
    <!-- Reset siempre disponible (no solo cuando el circuit breaker está abierto) -->
    <div style="margin-top:24px;padding-top:16px;border-top:1px solid #ECEFF5;text-align:center;">
      <p style="font-size:12px;color:#5B6478;margin-bottom:8px;">
        ¿Vinculación trabada o errores persistentes?
      </p>
      <button id="reset-btn" style="background:#F1F5F9;color:#0451DD;border:1px solid #D9DEEA;padding:8px 18px;border-radius:6px;font-weight:600;font-size:13px;cursor:pointer;">
        🔄 Forzar reset (limpiar y reintentar)
      </button>
      <div id="reset-status" style="font-size:12px;margin-top:8px;display:none;"></div>
    </div>
...
<script>
  ...
  function render(s) {
    ...
    // Error si lo hay
    if (s.lastError && Date.now() - s.lastError.at < 300000) {  // 5 min
    ...
    const circuitDetail = document.getElementById('circuit-detail');
    if (s.circuit && s.circuit.open) {
      circuitBanner.style.display = '';
      const mins = Math.ceil(s.circuit.opensIn / 60000);
      circuitDetail.innerHTML =
        'Se detectaron múltiples fallos de pairing seguidos (síntoma de ban). ' +
        'El bot pausó automáticamente todos los reintentos por <strong>' + mins + ' minutos</strong> ' +
        'para evitar que WhatsApp banee definitivamente el número.';
    } else {
  ...
  // Handler del botón Reset (siempre visible)
  document.addEventListener('DOMContentLoaded', () => {
    const resetBtn = document.getElementById('reset-btn');
    if (!resetBtn) return;

    resetBtn.addEventListener('click', async () => {
      if (!confirm('Esto va a limpiar la sesión del bot y reintentar la vinculación desde cero.\\n\\nSolo confirmá si:\\n• Ya pasaron al menos 24h desde el último ban (si hubo)\\n• O si la vinculación está trabada con errores persistentes\\n\\n¿Continuar?')) return;

      const originalText = resetBtn.textContent;
      resetBtn.disabled = true;
      resetBtn.textContent = '⏳ Reiniciando bot...';

      const status = document.getElementById('reset-status');
      status.style.display = 'none';

      try {
        const r = await fetch('/api/reset?token=' + encodeURIComponent(TOKEN), { method: 'POST' });
        const data = await r.json();
        status.style.display = '';
        if (r.ok) {
          status.style.color = '#047857';
          status.textContent = '✓ Reset OK. Esperando código nuevo en ~30s...';
