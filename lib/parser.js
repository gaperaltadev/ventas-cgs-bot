...
export function parseIntent(text, session = {}) {
  ...
  if (session?.flowStep === FLOW.PEDIDO_ESPERANDO_CLIENTE
      && !PEDIDO_FLOW_ESCAPE_RE.test(t) && !t.startsWith('!')) {
    return { command: '__pedido_buscar_cliente__', args: t.split(/\s+/) };
  }
  if (session?.flowStep === FLOW.PEDIDO_ALTA_NOMBRE
      && !KNOWN_COMMANDS_RE.test(t) && !t.startsWith('!')) {
    return { command: '__pedido_alta_nombre__', args: t.split(/\s+/) };
  }
  // Legacy
  if (session?.flowStep === FLOW.PEDIDO_ALTA_CLIENTE
      && !KNOWN_COMMANDS_RE.test(t) && !t.startsWith('!')) {
    return { command: '__pedido_alta_nombre__', args: t.split(/\s+/) };
  }
  if (session?.flowStep === FLOW.PEDIDO_ESPERANDO_ITEM
      && !PEDIDO_FLOW_ESCAPE_RE.test(t) && !t.startsWith('!')) {
    return { command: '__pedido_esperando_item__', args: t.split(/\s+/) };
  }
  // Legacy
  if (session?.flowStep === FLOW.PEDIDO_ESPERANDO_ITEMS
      && !PEDIDO_FLOW_ESCAPE_RE.test(t) && !t.startsWith('!')) {
    return { command: '__pedido_esperando_item__', args: t.split(/\s+/) };
  }
  if (session?.flowStep === FLOW.PEDIDO_ESPERANDO_CANTIDAD
      && !KNOWN_COMMANDS_RE.test(t) && !t.startsWith('!')) {
    return { command: '__pedido_esperando_cantidad__', args: t.split(/\s+/) };
  }
  if (session?.flowStep === FLOW.PEDIDO_CONFIRMANDO) {
...
function routeInteractive(id, session) {
  ...
  // Selección de producto desde lista (flujo pedido)
  if (id.startsWith('prod_')) {
    const prodId = id.slice(5);
    return { command: '__pedido_select_prod__', args: [prodId] };
  }

  // Catálogo interactivo — selección de categoría (cat_auto, cat_moto, ...)
  if (id.startsWith('cat_')) {
    return { command: '__catalogo_cat__', args: [id.slice(4)] };
  }

  // Catálogo interactivo — ficha de producto (ficha_20, ficha_25, ...)
  if (id.startsWith('ficha_')) {
    return { command: '__catalogo_ficha__', args: [id.slice(6)] };
  }
