...
import { sendToMeta, sendInteractiveButtons, sendInteractiveList } from './meta.js';
import { parseIntent, parseInteractive } from './parser.js';
import { handleCommand } from '../commands.js';
import { FLOW_COMMANDS }  from './constants.js';
import {
  getSession,
  setSession,
  clearSession,
...
// FLOW_COMMANDS importado desde lib/constants.js — fuente única de verdad.
...
// ════════════════════════════════════════════════════════════════════════════
// dispatch — envía la respuesta del handler al canal correcto de Meta.
//
// Tipos de resultado soportados:
//   string                           → texto plano
//   { text }                         → texto plano (legacy)
//   { _type:'buttons', body, buttons } → botones interactivos (2-3)
//   { _type:'list', body, buttonText, sections } → lista interactiva
// ════════════════════════════════════════════════════════════════════════════

async function dispatch(waPhone, result) {
  if (typeof result === 'string') {
    await sendToMeta(waPhone, result || FALLBACK_NO_COMMAND);
    return;
  }

  switch (result._type) {
    case 'buttons':
      await sendInteractiveButtons(waPhone, result.body, result.buttons);
      return;

    case 'list':
      await sendInteractiveList(waPhone, result.body, result.buttonText, result.sections);
      return;

    default:
      // Resultado con { text } o cualquier objeto sin _type
...
// ─── Extrae el contenido útil del payload JSON de Meta ───────────────────────
// message_body contiene JSON.stringify(messages[0]) — puede ser texto o interactivo.
// Devuelve { text, interactiveId } donde solo uno de los dos tiene valor.
function extractPayload(raw) {
  if (!raw) return { text: '', interactiveId: null };

  if (!raw.startsWith('{')) {
    // Mensaje legacy o plain-text directo — lo usamos tal cual
    return { text: raw, interactiveId: null };
  }

  try {
    const msg = JSON.parse(raw);
    if (msg.type === 'text') {
      return { text: msg.text?.body ?? '', interactiveId: null };
    }
    if (msg.type === 'interactive') {
      const id = msg.interactive?.button_reply?.id
              ?? msg.interactive?.list_reply?.id
              ?? null;
      return { text: '', interactiveId: id };
    }
    // Tipo no manejado (audio, imagen, sticker, etc.) → ignorar
    console.log(`[worker] tipo de mensaje no manejado: ${msg.type}`);
    return { text: '', interactiveId: null };
  } catch {
    // No era JSON válido — tratar como texto plano
    return { text: raw, interactiveId: null };
  }
}
...
async function processJob(job) {
  ...
  const { text, interactiveId } = extractPayload(job.message_body);

  console.log(`[worker] → procesando job ${job.id} | ${waPhone} | type=${interactiveId ? 'interactive' : 'text'} | "${interactiveId ?? text}"`);
  ...
  if (!allowed) {
    ...
    try {
      ...
      // Si Meta rechaza el envío (ej: número no registrado en cuenta test),
      // simplemente logueamos y continuamos — el job debe quedar como 'completado'
      // para no retrocesar en la cola.
      console.warn(`[worker]   no se pudo notificar a ${waPhone} (no autorizado):`, metaErr.message);
    }
    return;
  }

  // ─── 2. Prefijo requerido (salvo flujo activo o mensaje interactivo) ────────
  const session       = getSession(waPhone);
  const hasActiveFlow = !!(session.flowStep || session.lastResults?.length);

  // Los mensajes interactivos (botones/listas) no llevan prefijo — siempre pasan
  if (!interactiveId) {
  ...
  // ─── 3. Parsear intención ──────────────────────────────────────────────────
  const cleanText = !interactiveId && text.startsWith(PREFIX)
    ? text.slice(PREFIX.length).trim()
    : text;

  const { command, args } = interactiveId
    ? parseInteractive(interactiveId, session)
    : parseIntent(cleanText, session);
  ...
  let result;
  try {
    result = await handleCommand(command, args, session, waPhone);
  } catch (err) {
  ...
  // Persistir sesión antes de enviar (aplica a todos los tipos de resultado)
  if (result?._session) {
    setSession(waPhone, result._session);
  }

  await dispatch(waPhone, result);
}
